...
echo "------------------------------------"
echo "Stage 5: Wait for Instance"
echo "------------------------------------"
...
(
    ...
    aws ec2 wait instance-terminated --instance-ids "$INSTANCE_ID" >> $LOG_FILE 2>&1

    aws ec2 delete-security-group --group-id "$SG_ID" >> $LOG_FILE 2>&1

    echo "Cleanup done for $INSTANCE_ID" >> $LOG_FILE
) &

echo "------------------------------------"
echo "Stage 6: SSH Connect"
echo "------------------------------------"
